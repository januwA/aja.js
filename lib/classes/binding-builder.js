...
class BindingAttrBuilder extends BindingBuilder {
    constructor(node, attr, contextData) {
        ...
        this.attr = attr;
        this.contextData = contextData;
        if (this.name === const_string_1.formControlAttrName) {
            this._formControlSetup();
        }
        else if (this.name === const_string_1.formGroupAttrName && p_1.formp(this.node)) {
            const formGroup = core_1.getData(this.value, this.contextData);
            contextData.formGroup = formGroup;
        }
        else if (this.name === const_string_1.formControlNameAttrName) {
