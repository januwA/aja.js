    ...
    setup(contextData) {
        ...
        if (p_1.inputp(this.node) || p_1.textareap(this.node)) {
            if (p_1.checkboxp(this.node)) {
                ...
                const data = util_1.getData(this.modelAttr.value, contextData);
                mobx_1.autorun(() => {
                ...
                mobx_1.autorun(() => {
                    this._radioSetup(util_1.getData(this.modelAttr.value, contextData));
                });
                ...
                mobx_1.autorun(() => {
                    this._inputSetup(util_1.getData(this.modelAttr.value, contextData));
                });
            ...
            setTimeout(() => {
                mobx_1.autorun(() => {
                    this._selectSetup(util_1.getData(this.modelAttr.value, contextData));
                });
            });
        ...
        if (this.checkbox) {
            this.checkbox.addEventListener(const_string_1.EventType.change, () => {
                ...
                if (p_1.arrayp(data)) {
                    ...
                    if (this.modelAttr)
                        util_1.setData(this.modelAttr.value, this.checkbox.checked, contextData);
                }
    ...
    _radioChangeListener(contextData) {
        if (this.radio) {
            this.radio.addEventListener(const_string_1.EventType.change, () => {
                ...
                if (this.modelAttr)
                    util_1.setData(this.modelAttr.value, newData, contextData);
            });
        }
    }
    _inputSetup(value) {
        ...
        if (this.input) {
            ...
            this.input.addEventListener(const_string_1.EventType.input, () => {
                if (this.input && this.modelAttr) {
                    ...
                    util_1.setData(this.modelAttr.value, this.input.value, contextData);
                }
            });
        }
    }
            ...
            this.select.addEventListener(const_string_1.EventType.change, () => {
                if (this.select && this.modelAttr) {
                    ...
                    if (this.select.multiple) {
                        util_1.setData(bindKey, this.selectValues, contextData);
                    }
                    else {
                        util_1.setData(bindKey, this.select.value, contextData);
                    }
