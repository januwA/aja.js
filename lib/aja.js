...
class Aja {
    ...
    _attrBindHandle(node, { name, value }, contextData) {
        if (name === const_string_1.formControlName) {
            const formControl = core_1.getData(value, contextData);
            new form_control_service_1.FormControlSerivce(node, formControl);
        }
        else {
            ...
            switch (attrName) {
                case "style":
                    mobx_1.autorun(() => {
                        let data = core_1.getData(bindKey, contextData);
    ...
    _setTextContent(textNode, contextData) {
        ...
        if (!exp_1.interpolationExpressionExp.test(textBuilder.text))
            return;
        mobx_1.autorun(() => {
