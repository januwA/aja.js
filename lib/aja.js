    ...
    _parseBindAttrs(node, attrs, state) {
        for (const attr of attrs) {
            ...
            if (util_1.modelp(name, this._modeldirective)) {
                ...
                node.classList.add(aja_model_1.AjaModel.classes.untouched, aja_model_1.AjaModel.classes.pristine, aja_model_1.AjaModel.classes.valid);
                if (nodeName === "INPUT" || nodeName === "TEXTAREA") {
                    ...
                    if (inputNode.type === "checkbox") {
                        ...
                        if (!util_1.arrayp(data)) {
                            ...
                            store_1.reaction(() => [this._getData(value, state)], states => {
                                const data = states[0];
                                let ivalue = util_1.getCheckBoxValue(inputNode);
                        ...
                        store_1.reaction(() => [this._getData(value, state)], states => {
                            ...
                            if (inputNode.required && !value) {
                                // 控件的值无效
                                inputNode.classList.replace(aja_model_1.AjaModel.classes.valid, aja_model_1.AjaModel.classes.invalid);
                            }
                            else {
                                // 控件的值有效
                                inputNode.classList.replace(aja_model_1.AjaModel.classes.invalid, aja_model_1.AjaModel.classes.valid);
                            }
                        ...
                        inputNode.addEventListener("input", () => {
                            ...
                            inputNode.classList.replace(aja_model_1.AjaModel.classes.pristine, aja_model_1.AjaModel.classes.dirty);
                        });
                        // 控件被访问了
                        inputNode.addEventListener("blur", () => {
                            inputNode.classList.replace(aja_model_1.AjaModel.classes.untouched, aja_model_1.AjaModel.classes.touched);
                        });
                    }
                }
